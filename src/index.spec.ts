...
describe('CardCollect', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(document, 'getElementById').mockImplementation((id: string) => {
            switch (id) {
                case 'cc-holder': return holderNameMock;
                case 'cc-number': return cardNumberMock;
                case 'cc-expiration-date': return expiryDateMock;
                case 'cc-cvc': return cvvMock;
                default:
                    return null;
            }
        });
    ...
    it('When call cardCollect submit should fix expiration date value', async () => {
        ...
        const holder: HTMLInputElement = holderNameMock.getElementsByTagName('input')[0] as HTMLInputElement;
        holder.value = 'Paybyrd';
        const cardNumber: HTMLInputElement = cardNumberMock.getElementsByTagName('input')[0] as HTMLInputElement;
        cardNumber.value = '5555341244441115';
        const expiryDate: HTMLInputElement = expiryDateMock.getElementsByTagName('input')[0] as HTMLInputElement;
